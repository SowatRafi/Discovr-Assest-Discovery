...
| Active Directory | Password captured by sniffing or by an attacker in the middle; account lockout | A simple bind (which carries the password) only happens inside a TLS channel whose certificate and hostname are **verified** (system trust store, or the desktop CA certificate field with the domain CA); otherwise NTLM challenge-response, which never sends the password; one bind attempt per mechanism; password entered in a masked native field, never logged, stored or echoed |
...
| Target networks | Disruption of fragile devices | Bounded TCP connects and timeouts (Gentle intensity under More scan options); Standard mode also reads SSH banners, bounded HTTP responses and targeted unicast UPnP descriptions. No authentication, redirects, multicast searches or UPnP control requests. Passive mode sends nothing. |
...
- Inventory lives in memory while the desktop runs; files are written only when you save or
  export through a native file dialog. Reports contain IPs, hostnames, MAC
...
- **Unverifiable domain controllers get NTLM.** When the DC certificate cannot be verified
  ...
  active attacker could relay the NTLM exchange. Choose the domain CA in **CA certificate** to get a
