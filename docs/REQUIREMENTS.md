...
| Minimal topology knowledge, preferably unprivileged | Startup local IPv4 display, interface choice, prepared subnet, editable scope, ordinary TCP sockets/cache reads | OS route selects the usual connection; isolated LANs work without internet. Ambiguous connections require a choice, missing masks use host-only scope, and no scan starts automatically. Missing documentation cannot reveal inaccessible networks. |
